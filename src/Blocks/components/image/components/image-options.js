	...
	return (
		<>
			...
			{imageUse &&
				<>
					{showImageUrl &&
						<Responsive label={<IconLabel icon={icons.image} label={__('Image source', 'petar-bjelica-infinum')} />}>
							{Object.keys(checkAttrResponsive('imageUrl', attributes, manifest)).map(function (keyName) {
								...
								if (point === 'Large') {
									point = '';
									pointLabel = __('Default (all screens)', 'petar-bjelica-infinum');
								}
					...
					{showImageAlt &&
						<TextareaControl
							label={<IconLabel icon={icons.altText} label={__('Alt text', 'petar-bjelica-infinum')} />}
							value={imageAlt}
					...
					{showImageFull &&
						<IconToggle
							icon={icons.size}
							label={__('Fill container', 'petar-bjelica-infinum')}
							help={__('Span the full width of the container and ignore its maximum width', 'petar-bjelica-infinum')}
							checked={imageFull}
